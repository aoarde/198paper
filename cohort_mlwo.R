...
sequences = rep(as.numeric(c_mlwo$Activity_Group), as.numeric(c_mlwo$Duration))
...
data_mlwo <- as.data.frame(seq, row.names = unique(c_mlwo$ID))

# Use created variable names to name the columns in the dataframe
colnames(data_mlwo) <- activities

# Create ID
data_mlwo$id <- as.numeric(row.names(data_mlwo))
...
final_mlwo <- as_tibble(data_mlwo) 

# Define the sequence object
seq_mlwo <- seqdef(final_mlwo,
                   var = activities,  
...
seqiplot(seq_mlwo, border = NA, with.legend = "right", legend.prop=0.4)
seqIplot(seq_mlwo, border = NA, with.legend = "right", legend.prop=0.4, idxs = 1:4)

seqdplot(seq_mlwo, border = NA, with.legend = "right", legend.prop=0.4)

### Transitions
# transitions from state to state (in probabilities)
trate <- seqtrate(seq_mlwo)
round(trate, 2)
...
time15_seq_mlwo <- seqgranularity(seq_mlwo, tspan=15, method="mostfreq")
## plot the tempogram
seqdplot(time15_seq_mlwo, border = NA, with.legend = "right", legend.prop=0.4)
...
seqHtplot(time15_seq_mlwo, with.legend = "right", legend.prop=0.4)
...
scost <- seqsubm(time15_seq_mlwo, method = "TRATE")
round(scost, 3)
...
ccost <- seqsubm(time15_seq_mlwo, method="CONSTANT", cval=2)
round(ccost, 3)