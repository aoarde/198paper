...
c_wla <- VNMMain %>% filter(VNMMain$COHORT == "Women living alone")
  
# Create sequences of activities per activity (result: long list of combined sequences)
sequences = rep(as.numeric(c_wla$Activity_Group), as.numeric(c_wla$Duration))
  ...
  data_wla <- as.data.frame(seq, row.names = unique(c_wla$ID))
  
  # Use created variable names to name the columns in the dataframe
  colnames(data_wla) <- activities
  
  # Create ID
  data_wla$id <- as.numeric(row.names(data_wla))
...
final_wla <- as_tibble(data_wla) 

# Define the sequence object
seq_wla <- seqdef(final_wla,
              var = activities,  
...
seqiplot(seq_wla, border = NA, with.legend = "right", legend.prop=0.4)
seqIplot(seq_wla, border = NA, with.legend = "right", legend.prop=0.4, idxs = 1:4)

seqdplot(seq_wla, border = NA, with.legend = "right", legend.prop=0.4)

### Transitions
# transitions from state to state (in probabilities)
trate <- seqtrate(seq_wla)
round(trate, 2)
...
time15_seq_wla <- seqgranularity(seq_wla, tspan=15, method="mostfreq")
## plot the tempogram
seqdplot(time15_seq_wla, border = NA, with.legend = "right", legend.prop=0.4)
...
seqHtplot(time15_seq_wla, with.legend = "right", legend.prop=0.4)
...
scost <- seqsubm(time15_seq_wla, method = "TRATE")
round(scost, 3)
...
ccost <- seqsubm(time15_seq_wla, method="CONSTANT", cval=2)
round(ccost, 3)