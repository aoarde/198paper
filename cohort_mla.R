...
c_mla <- VNMMain %>% filter(VNMMain$COHORT == "Men living alone")

# Create sequences of activities per activity (result: long list of combined sequences)
sequences = rep(as.numeric(c_mla$Activity_Group), as.numeric(c_mla$Duration))
...
data_mla <- as.data.frame(seq, row.names = unique(c_mla$ID))

# Use created variable names to name the columns in the dataframe
colnames(data_mla) <- activities

# Create ID
data_mla$id <- as.numeric(row.names(data_mla))
...
final_mla <- as_tibble(data_mla) 

# Define the sequence object
seq_mla <- seqdef(final_mla,
                  var = activities,  
...
seqiplot(seq_mla, border = NA, with.legend = "right", legend.prop=0.4)
seqIplot(seq_mla, border = NA, with.legend = "right", legend.prop=0.4, idxs = 1:4)

seqdplot(seq_mla, border = NA, with.legend = "right", legend.prop=0.4)

### Transitions
# transitions from state to state (in probabilities)
trate <- seqtrate(seq_mla)
round(trate, 2)
...
time15_seq_mla <- seqgranularity(seq_mla, tspan=15, method="mostfreq")
## plot the tempogram
seqdplot(time15_seq_mla, border = NA, with.legend = "right", legend.prop=0.4)
...
seqHtplot(time15_seq_mla, with.legend = "right", legend.prop=0.4)
...
scost <- seqsubm(time15_seq_mla, method = "TRATE")
round(scost, 3)
...
ccost <- seqsubm(time15_seq_mla, method="CONSTANT", cval=2)
round(ccost, 3)
