...
# Create a list to store dataframes for each cohort
cohort_data_list <- list()

# Loop through each unique cohort in VNMMain$COHORT
for (cohort in unique(VNMMain$COHORT)) {
  # Filter the VNMMain data for the current cohort
  cohort_data <- VNMMain[VNMMain$COHORT == cohort, ]
  
  # Create sequences of activities per activity (result: long list of combined sequences)
  sequences = rep(as.numeric(cohort_data$Activity_Group), as.numeric(cohort_data$Duration))
  ...
  data <- as.data.frame(seq, row.names = unique(cohort_data$ID))
  ...
  data$Cohort <- cohort  # This will add the cohort name to the dataframe
  
  # Store the dataframe in the list
  cohort_data_list[[cohort]] <- data
}

# Combine all cohort dataframes into one if needed
final_data <- do.call(rbind, cohort_data_list)
